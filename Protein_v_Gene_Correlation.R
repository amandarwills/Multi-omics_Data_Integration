...
p <- xyplot(data.filtered$log.protein.count ~ data.filtered$log.gene.count | data.filtered$TP_Condition , 
       aspect = 1:1,
       panel = function(x, y, ...) {
         panel.xyplot(x, y, ...)
         fm <- lm(y ~ x - 0)
         panel.lines(x, fitted(fm), col.line = "red", ...)
         panel.text(11.5,1, labels=sprintf("y = %.3f x + %.3f\nR² = %.2f\nnPoints = %.0f", 
...
trellis.device(device="pdf", file="xyplot.pdf", paper="a4", height=8.3, width=11.7)
print(p)
dev.off()
